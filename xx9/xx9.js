(function () {
  ...
  // --- Library Hero carousel: CSS-driven infinite marquee (Codea/Sublime style). Track with 2 segments, keyframe animation. ---
  var HERO_CAROUSEL_MAX_HEIGHT = 186;
  var heroCarousel = document.getElementById('library-hero-carousel');
  if (heroCarousel) {
    var carouselWidths = [200, 160, 220, 180, 140, 190, 170, 210, 150, 230, 185, 165, 195];
    var HERO_ITEM_COUNT = 24;
    var shuffled = shuffleArray(staffPicksFeedCards.slice());
    var heroItems = [];
    for (var h = 0; h < HERO_ITEM_COUNT; h++) {
      heroItems.push(shuffled[h % shuffled.length]);
    }
        ...
        if (item.type === 'image' || item.type === 'file') {
          ...
          if (img) {
            function sizeWrapFromImage() {
              ...
              var cardH = Math.min(cardW * (nh / nw), HERO_CAROUSEL_MAX_HEIGHT);
          ...
          if (video) {
            function sizeWrapFromVideo() {
              ...
              if (!vw || !vh) return;
              var cardW = parseFloat(wrap.style.width, 10);
              var cardH = Math.min(cardW * (vh / vw), HERO_CAROUSEL_MAX_HEIGHT);
              wrap.style.height = cardH + 'px';
            }
    ...
    track.className = 'library-hero-carousel-track';
    track.setAttribute('role', 'list');
    var segment1 = document.createElement('div');
    segment1.className = 'library-hero-carousel-segment';
    segment1.setAttribute('aria-hidden', 'true');
    var segment2 = document.createElement('div');
    segment2.className = 'library-hero-carousel-segment';
    segment2.setAttribute('aria-hidden', 'true');
    segment1.appendChild(buildHeroSegment());
    segment2.appendChild(buildHeroSegment());
    track.appendChild(segment1);
    track.appendChild(segment2);
    heroCarousel.appendChild(track);
    requestAnimationFrame(function () {
      heroCarousel.querySelectorAll('.sublime-card--text, .sublime-card--highlight').forEach(hideExpandIfNotTruncated);
    });
    var HERO_ANIMATION_DURATION_S = 180;
    var HERO_PAUSE_AFTER_USER_MS = 2500;
    var heroPauseTimeoutId = null;
    function heroPauseAnimation() {
      track.classList.add('library-hero-carousel-track--paused');
      if (heroPauseTimeoutId) clearTimeout(heroPauseTimeoutId);
      heroPauseTimeoutId = setTimeout(function () {
        var trackWidth = track.offsetWidth;
        var segmentWidth = trackWidth / 2;
        if (segmentWidth > 0) {
          ...
          if (transform && transform !== 'none') {
            ...
            if (m) {
              var parts = m[1].split(',');
              if (parts.length >= 6) tx = parseFloat(parts[4], 10);
            }
          }
          var totalOffset = -tx + heroCarousel.scrollLeft;
          var progress = (totalOffset / segmentWidth) % 1;
          if (progress < 0) progress += 1;
          track.style.animationDelay = -(progress * HERO_ANIMATION_DURATION_S) + 's';
          heroCarousel.scrollLeft = 0;
        }
        track.classList.remove('library-hero-carousel-track--paused');
        heroPauseTimeoutId = null;
      }, HERO_PAUSE_AFTER_USER_MS);
    }
    heroCarousel.addEventListener('wheel', heroPauseAnimation, { passive: true });
    heroCarousel.addEventListener('touchstart', heroPauseAnimation, { passive: true });
    heroCarousel.addEventListener('touchmove', heroPauseAnimation, { passive: true });
  }
  ...
  var LIBRARY_SECTIONS_STORAGE_KEY = 'xx9-library-sections';
