(function () {
  ...
  // --- xx12: Floating butterfly — only on My Library homepage; smooth flight; no-fly zones ---
  (function () {
    var layer = document.getElementById('butterfly-layer');
    var link = document.getElementById('butterfly-link');
    ...
    var butterflySize = 80;
    var speed = 58;
    var angle = Math.random() * Math.PI * 2;
    var targetAngle = angle;
    var angleSmooth = 2.2; // lerp toward target per second (smooth curves)
    var noFlyPadding = 10;
    var noFlySelectors = ['#library-top-bar', '.library-view-inner'];

    var maxX = Math.max(0, window.innerWidth - butterflySize);
    var maxY = Math.max(0, window.innerHeight - butterflySize);
    var x = Math.random() * maxX;
    var y = Math.random() * maxY;
    var lastTime = null;
    var paused = false;
    var nextTurnAt = 0;
    var turnIntervalMin = 600;
    var turnIntervalMax = 1800;

    function randomInRange(lo, hi) {
      return lo + Math.random() * (hi - lo);
    }

    function isOnLibraryHomepage() {
      if (!libraryView || !libraryView.classList.contains('library-view--visible')) return false;
      if (collectionView && collectionView.classList.contains('collection-view--visible')) return false;
      if (allCardsView && allCardsView.classList.contains('all-cards-view--visible')) return false;
      if (collectionsDirectoryView && collectionsDirectoryView.classList.contains('collections-directory-view--visible')) return false;
      return true;
    }

    function getNoFlyRects() {
      var rects = [];
      noFlySelectors.forEach(function (sel) {
        var el = document.querySelector(sel);
        if (!el) return;
        var r = el.getBoundingClientRect();
        rects.push({
          left: r.left - noFlyPadding,
          top: r.top - noFlyPadding,
          right: r.right + noFlyPadding,
          bottom: r.bottom + noFlyPadding
        });
      });
      return rects;
    }

    function overlapAmount(ax, ay, bx, by, zone) {
      if (ax >= zone.right || ax + butterflySize <= zone.left) return null;
      if (ay >= zone.bottom || ay + butterflySize <= zone.top) return null;
      var pushLeft = (ax + butterflySize) - zone.left;
      var pushRight = zone.right - ax;
      var pushTop = (ay + butterflySize) - zone.top;
      var pushBottom = zone.bottom - ay;
      var best = { d: Infinity };
      if (pushLeft > 0 && pushLeft < best.d) best = { d: pushLeft, dx: -pushLeft, dy: 0, reflect: 'v' };
      if (pushRight > 0 && pushRight < best.d) best = { d: pushRight, dx: pushRight, dy: 0, reflect: 'v' };
      if (pushTop > 0 && pushTop < best.d) best = { d: pushTop, dx: 0, dy: -pushTop, reflect: 'h' };
      if (pushBottom > 0 && pushBottom < best.d) best = { d: pushBottom, dx: 0, dy: pushBottom, reflect: 'h' };
      return best.d === Infinity ? null : best;
    }

    function applyBoundaries() {
      var rects = getNoFlyRects();
      for (var i = 0; i < rects.length; i++) {
        var o = overlapAmount(x, y, 0, 0, rects[i]);
        while (o) {
          x += o.dx;
          y += o.dy;
          if (o.reflect === 'v') targetAngle = Math.PI - targetAngle + randomInRange(-0.15, 0.15);
          else targetAngle = -targetAngle + randomInRange(-0.15, 0.15);
          angle = targetAngle;
          o = overlapAmount(x, y, 0, 0, rects[i]);
        }
      }
      maxX = Math.max(0, window.innerWidth - butterflySize);
      maxY = Math.max(0, window.innerHeight - butterflySize);
      if (x < 0) { x = 0; targetAngle = Math.PI - targetAngle + randomInRange(-0.2, 0.2); angle = targetAngle; }
      if (y < 0) { y = 0; targetAngle = -targetAngle + randomInRange(-0.2, 0.2); angle = targetAngle; }
      if (x > maxX) { x = maxX; targetAngle = Math.PI - targetAngle + randomInRange(-0.2, 0.2); angle = targetAngle; }
      if (y > maxY) { y = maxY; targetAngle = -targetAngle + randomInRange(-0.2, 0.2); angle = targetAngle; }
    }
    ...
    function tick(now) {
      ...
      var onHome = isOnLibraryHomepage();
      layer.style.visibility = onHome ? 'visible' : 'hidden';
      if (!onHome) {
      ...
      if (now >= nextTurnAt) {
        nextTurnAt = now + randomInRange(turnIntervalMin, turnIntervalMax);
        targetAngle += randomInRange(-0.5, 0.5);
        var nearestHorizontal = Math.cos(targetAngle) >= 0 ? 0 : Math.PI;
        targetAngle = targetAngle * 0.82 + nearestHorizontal * 0.18;
      }

      var blend = 1 - Math.exp(-angleSmooth * dt);
    ...
    link.style.left = x + 'px';
    link.style.top = y + 'px';
    layer.style.visibility = isOnLibraryHomepage() ? 'visible' : 'hidden';
    nextTurnAt = performance.now() + randomInRange(turnIntervalMin, turnIntervalMax);

    link.addEventListener('mouseenter', function () { paused = true; });
    link.addEventListener('mouseleave', function () { paused = false; lastTime = null; });
